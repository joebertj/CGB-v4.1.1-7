...
Func ClearObstacles()
	...
	For $obsCnt = 0 To 7
		...
		$res = DllCall($LibDir & "\ImageSearch.dll", "str", "searchTile", "handle", $sendHBitmap, "str", $obstacles[$obsCnt], "float", 0.910)
		...
		;SetLog("DLL Call succeeded " & $res[0], $COLOR_RED)
			...
			Else
				...
				$ObstacleX = Int($expRet[1])
				$ObstacleY = Int($expRet[2])
				If isInsideBigDiamondXY($ObstacleX, $ObstacleY) Then					
					...
					If _ColorCheck(_GetPixelColor(432, 592), Hex(0xE82CD0, 6), 20) Then ; Check Elix
						SetLog("Obstacle Type " & $obsCnt & " found (" & $ObstacleX & "," & $ObstacleY &")", $COLOR_GREEN)
						If _Sleep(500) Then Return
					...
					If _Sleep(500) Then Return
				ElseIf $res[0] > 1 Then
					$ObstacleX = Int($expRet[3])
					$ObstacleY = Int($expRet[4])
					If isInsideBigDiamondXY($ObstacleX, $ObstacleY) Then						
						If _Sleep(500) Then Return
						Click($ObstacleX, $ObstacleY,1,0,"#0120")
						If _Sleep(1000) Then Return
