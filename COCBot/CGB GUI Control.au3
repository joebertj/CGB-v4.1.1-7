...
;~ Control Tab Files
;~ ------------------------------------------------------
#include "GUI\CGB GUI Control Bottom.au3"
#include "GUI\CGB GUI Control Tab General.au3"
#include "GUI\CGB GUI Control Tab Troops.au3"
#include "GUI\CGB GUI Control Tab Search.au3"
#include "GUI\CGB GUI Control Tab Attack.au3"
#include "GUI\CGB GUI Control Tab Advanced.au3"
...
#include "GUI\CGB GUI Control Toolbox.au3"
...
Func copySettingsToClip()
  ...
  $settingsStr = "[spoiler=""My Settings""]" & $configSettings & "=============" & @CRLF & $buildingSettings & "[/spoiler]"
  ClipPut ($settingsStr)
...
Func importSettingsFromClip()
  ...
  if MsgBox($MB_YESNO, "Are you sure", "This will overwrite your current settings with what you've copied to your clipboard, are you sure? (it is recommended you make copies of your settings first or switch profiles)") = $IDYES Then
      $settingsArr = StringSplit(ClipGet(), "=============" & @CRLF, $STR_ENTIRESPLIT)
      if UBound($settingsArr) = 3 then
         $hFile = FileOpen($config, $FO_OVERWRITE)
         FileWrite($hFile, $settingsArr[1])
         FileClose($hFile)

         $hFile = FileOpen($building, $FO_OVERWRITE)
         FileWrite($hFile, $settingsArr[2])
         FileClose($hFile)

         MsgBox($MB_SYSTEMMODAL, "Restart", "New settings were written, the bot will close and the new settings will be ready when you start the bot")
         Exit
      Else
